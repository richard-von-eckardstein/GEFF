...
class GEF:
    ...
    def EoMF(x, dlnkhdt):
        ...
        ScalarCpl = (x.dIdphi()*x.vals["dphi"]+aAlpha*x.vals["sigmaB"]*(1-x.Ferm2))
        
        damp = np.zeros(bdrF.shape)# np.array([[(scale)**(i+4)*(1.-dampscale)*(Whitt[j,0] + (-1)**i*Whitt[j,1])/(i+4) for j in range(3)] for i in range(x.ntr)])/(4*np.pi**2)
