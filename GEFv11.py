...
class GEF:
    ...
    def CheckFermionEntry(x, dlnkhdtO, dlnkhdtE, dlnkhdtL):
        if(x.Ferm2==1):
            khO = x.vals["khO"]
            kS = x.vals["kS"]
            kappa = np.log(kS/khO)
    ...
    def EoMrhoChi(x):
        return (x.vals["a"]**(x.alpha)*(x.vals["sigmaE"]*x.vals["E"][0]
                                        - x.vals["sigmaB"]*(1-x.Ferm2)*x.vals["G"][0])- 4*x.vals["H"]*x.vals["rhoChi"])
    ...
    def TimeStep(x, t, y):
        ...
        if (x.SE != None):
            if (x.AltDamp == 1):
                ...
                dydt[5] = x.EoMlnkh(dydt[2], x.vals["khL"])
                dlnkhdt, x.vals["kh"], x.vals["delta"] = x.CheckFermionEntry(dydt[3], dydt[4], dydt[5])
                if (x.Ferm2 == 0):
    ...
    def RunGEF(x, t0=0., t1=120., atol=1e-6, rtol=1e-3):
        if not(x.completed):
            ...
            for i in range(len(t)):
                ...
                if (x.AltDamp == 2):
                    #print(x.Ferm2)
                    _, x.vals["kh"], x.vals["delta"] = x.CheckFermionEntry(0., 0., 0.)
