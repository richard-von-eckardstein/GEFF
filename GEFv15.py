...
class GEF:
    def __init__(x, alpha, beta, Mpl, ini, M, ntr, SE, AltDamp=0, approx=True, order=15):
        x.units = True
        x.completed = False
        x.alpha = alpha
        x.beta = beta
    ...
    def ComputeImprovedSigmaKDep(x):
        ...
        if Sigma==0:
            ...
            ks = gmu**(1/2)*Eprime**(1/2)*a**(1-x.alpha)/gamma
    ...
    def TaylorWhitt(x):
        ...
        Y = {"E":np.zeros((order, 2)), "B":np.zeros((order,2)), "G":np.zeros((order,2))}
    ...
    def EoMrhoChi(x):
        ...
        for name in taylor.keys():
            for i in range(x.order):
                for j in range(i+1):
                    damp[name] += (-1)*(ys)**(i+1)/(i+1)*scale**(4)*(taylor[name][j,0] + taylor[name][j,1])*coeff[i-j]/(4*np.pi**2)*x.Ferm2
