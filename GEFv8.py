###Working Version for SE=-1, includes Boundary Corrections and Damping Corrections (properly) for sigmaE
...
class GEF:
    ...
    def EoMF(x, dlnkhdt):
        ...
        """dampE = np.array([max((E[i] - a*((scale)**(i+4)-(dampscale)**(i+4))*(Whitt[0,0] + (-1)**i*Whitt[0,1])/(4*np.pi**2)*x.Ferm2/(i+4))*np.sign(E[i]), 0.)*np.sign(E[i]) for i in range(x.ntr)]) 
        dampB = np.array([max((B[i] - a*((scale)**(i+4)-(dampscale)**(i+4))*(Whitt[1,0] + (-1)**i*Whitt[1,1])/(4*np.pi**2)*x.Ferm2/(i+4))*np.sign(B[i]), 0.)*np.sign(B[i]) for i in range(x.ntr)]) 
        dampG = np.array([max((G[i] - a*((scale)**(i+4)-(dampscale)**(i+4))*(Whitt[2,0] + (-1)**i*Whitt[2,1])/(4*np.pi**2)*x.Ferm2/(i+4))*np.sign(G[i]), 0.)*np.sign(G[i]) for i in range(x.ntr)]) 
        
        
        if( any(dampE) != 0.):
            print(x.vals["t"], dampE/E)
        ...
        #print(x.vals["t"], 1-damp[0,0]/E[0], 1-damp[0,2]/G[0], x.Ferm2)
